    ...
    @section('title', 'Editar disponibilidad médica')
        ...
        <div class="container-fluid px-4">
            <div class="row g-3 justify-content-between mb-3">
                <div class="mt-4 text-center">
                    <h5>Editar disponibilidad médica</h5>
                </div>
                <ol class="breadcrumb mb-4">
                    <li class="breadcrumb-item"><a href="{{ route('panel') }}">Inicio</a></li>
                    <li class="breadcrumb-item"><a href="{{ route('disponibilidades.index') }}">Disponibilidad</a></li>
                    <li class="breadcrumb-item active">Editar disponibilidad médica</li>
                </ol>
            </div>
                                ...
                                <form action="{{ route('disponibilidades.update', [$disponibilidad->id]) }}" method="post">
                                    ...
                                    <div class="mb-4">
                                        ...
                                        <select name="medico_especialidad_id" id="medico_especialidad_id"
                                            ...
                                            aria-label="form-control medico_especialidad_id">
                                            @foreach ($medicos as $item)
                                                <option value="{{ $item->id }}"
                                                    {{ old('medico_especialidad_id', $disponibilidad->medico_especialidad_id) == $item->id ? 'selected' : '' }}>
                                                    {{ $item->usuario->name . ' - ' . $item->especialista->nombre }}
                                                </option>
                                            @endforeach
                                        </select>
