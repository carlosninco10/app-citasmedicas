        ...
        <div class="container-fluid px-4">
            <div class="row g-3 justify-content-between mb-3">
                <div class="mt-4 text-center">
                    <h5>Disponibilidades médicas</h5>
                </div>
                <ol class="breadcrumb mb-4">
                    <li class="breadcrumb-item"><a href="{{ route('panel') }}">Inicio</a></li>
                    <li class="breadcrumb-item active">Disponibilidades</li>
                </ol>
            </div>
            <div class="mb-4">
                <a class="btn btn-primary" href="{{ route('disponibilidades.create') }}" role="button">Añadir nuevo
                    registro</a>
            </div>
            <div class="card mb-4">
                <div class="card-header">
                    <i class="fas fa-table me-1"></i>
                    Tabla disponibilidades médicas
                </div>
                <div class="card-body">
                        ...
                        <table id="datatablesSimple" class="table table-striped">
                            ...
                            <tbody>
                                @foreach ($disponibilidades as $item)
                                    <tr>
                                        <td>{{ $item->medicoEspecialista->usuario->name }}</td>
                                        <td>{{ $item->medicoEspecialista->especialista->nombre }}</td>
                                        <td>{{ \Carbon\Carbon::parse($item->fecha)->format('d-m-Y') }}</td>
                                        <td>{{ \Carbon\Carbon::parse($item->hora_inicio)->format('h:i A') }}</td>
                                        <td>{{ \Carbon\Carbon::parse($item->hora_fin)->format('h:i A') }}</td>
                                        <td>
                                            @if ($item->estado == 1)
                                            ...
                                            <div class="container-fluid">
                                                <div class="row">
                                                    <div class="col-md-4 me-1">
                                                        <form action="{{ route('disponibilidades.edit', [$item->id]) }}"
                                                            method="GET">
                                        ...
                                        <div class="modal-dialog">
                                            <div class="modal-content">
                                                ...
                                                <div class="modal-body">
                                                    {{ $item->estado == 1 ? '¿Seguro que quieres eliminar el disponibilidad?' : '¿Seguro que quieres restaurar el usuario?' }}
                                                    {{ $item->id }}
                                                </div>
                                                <div class="modal-footer">
                                                    <button type="button" class="btn btn-secondary"
                                                        data-bs-dismiss="modal">Cerrar</button>
                                                    <form action="{{ route('disponibilidades.destroy', [$item->id]) }}"
                                                        method="post">
                                                        @method('DELETE')
                                                        @csrf
